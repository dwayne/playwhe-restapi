...
module PlayWhe
  module UseCases
    class GetMarkCountsForWeekday
      attr_reader :results_dataset, :opts

      SCHEMA = Dry::Validation.Form do
        optional(:weekday).maybe(:int?, gteq?: 1, lteq?: 6)
      ...
      def call
        ...
        if result.success?
          ...
          data[:weekday] = current_time.wday unless data[:weekday]
          data[:by_period] = !!data[:by_period]

          ds = results_dataset.mark_counts_for_weekday(data[:weekday], data[:by_period])
      ...
      def generate_response(ds, data)
        response = {
          self: "/stats/mark-counts-for-weekday#{as_query(data)}"
        }
